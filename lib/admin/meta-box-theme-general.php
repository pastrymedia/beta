...
add_action( 'admin_menu', 'beta_theme_admin_general' );

function beta_theme_admin_general() {
	...
	add_filter( "sanitize_option_{$prefix}_theme_settings", 'beta_theme_validate_general' );

	/* Adds my_help_tab when my_admin_page loads */
    add_action('load-'.$theme_settings_page, 'beta_theme_settings_general_help');
}
 ...
 * @return void
 */
function beta_theme_settings_general() {

	add_meta_box(
		'beta-theme-general',			// Name/ID
		__( 'Header and Footer Scripts', 'beta' ),	// Label
		'beta_meta_box_theme_display_general',			// Callback function
 ...
 */
function beta_theme_validate_general( $settings ) {
 ...
 */
function beta_theme_settings_general_help() {

	$screen = get_current_screen();

	$general_help =
		'<h3>' . __( 'Header and Footer Scripts', 'beta' ) . '</h3>' .
		'<p>'  . __( 'This provides you with two fields that will output to the head section of your site and just before the closing body tag. These will appear on every page of the site and are a great way to add analytic code, Google Font and other scripts. You cannot use PHP in these fields.', 'beta' ) . '</p>';
			
	$screen->add_help_tab( array(
		'id'      => 'beta-settings' . '-general',
		'title'   => __( 'Header and Footer Scripts', 'beta' ),
