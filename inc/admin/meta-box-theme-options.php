...
add_action( 'admin_menu', 'beta_theme_admin_options' );

function beta_theme_admin_options() {
	...
	/* Enqueue styles */
	add_action( 'admin_enqueue_scripts', 'beta_admin_scripts' );

	/* Adds my_help_tab when my_admin_page loads */
    add_action('load-'.$theme_settings_page, 'beta_theme_settings_help');
}
...
function beta_admin_scripts( $hook_suffix ) {
    
    global $theme_settings_page;
	
    if ( $theme_settings_page == $hook_suffix ) {

	    wp_enqueue_script( 'beta-admin', get_template_directory_uri() . '/inc/js/beta-admin.js', array( 'jquery' ), '20121231', false );
	    wp_enqueue_style( 'beta-admin', get_template_directory_uri() . '/inc/css/beta-admin.css' );
...
function beta_theme_settings_help() {
	...
	$screen->add_help_tab( array(
		...
		'title'   => __( 'Customize', 'beta' ),
		'content' => $customize_help,
	) );

	//* Add help sidebar
	$screen->set_help_sidebar(
		'<p><strong>' . __( 'For more information:', 'beta' ) . '</strong></p>' .
		'<p><a href="http://themehall.com/contact" target="_blank" title="' . __( 'Get Support', 'beta' ) . '">' . __( 'Get Support', 'beta' ) . '</a></p>'
	);
